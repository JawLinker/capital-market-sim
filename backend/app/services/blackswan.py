"""Random era-flavored black swan events that shake up a playthrough."""

import random

BLACK_SWANS = [
    {
        "id": "bs01",
        "from_date": "2021-03-01",
        "to_date": "2021-06-30",
        "sentiment_delta": 0.05,
        "title_zh": "北向资金异动",
        "title_en": "Northbound Money Stir",
        "prose_zh": "北向资金单日大幅净流入，龙虎榜上外资席位集体出现，市场传说“聪明钱回来了”。",
        "prose_en": "Northbound money floods in for the day, foreign seats crowd the rankings, and the market whispers that smart money is back.",
    },
    {
        "id": "bs02",
        "from_date": "2021-09-01",
        "to_date": "2021-12-31",
        "sentiment_delta": -0.08,
        "title_zh": "限电潮来袭",
        "title_en": "The Power-Cut Wave",
        "prose_zh": "多省份发布限电通知，生产链股票集体下挫，段子手连夜写稿：电停了，梦也停了。",
        "prose_en": "Provinces issue power-cut orders, supply-chain stocks slump, and the memes write themselves overnight.",
    },
    {
        "id": "bs03",
        "from_date": "2022-01-01",
        "to_date": "2022-06-30",
        "sentiment_delta": -0.08,
        "title_zh": "全球通胀冲击",
        "title_en": "Global Inflation Shock",
        "prose_zh": "海外通胀数据超预期，加息预期升温，全球风险资产一起打了个哆嗦。",
        "prose_en": "Overseas inflation overshoots, rate-hike bets firm up, and risk assets around the world shiver at once.",
    },
    {
        "id": "bs04",
        "from_date": "2022-04-01",
        "to_date": "2022-12-31",
        "sentiment_delta": 0.05,
        "title_zh": "政策密集期",
        "title_en": "The Policy Season",
        "prose_zh": "稳增长政策密集落地，基建与地产链集体反弹，营业部里又开始排起开户长队。",
        "prose_en": "Growth-support policies land in rapid succession, infrastructure and property chains bounce, and account queues reform at the branches.",
    },
    {
        "id": "bs05",
        "from_date": "2023-02-01",
        "to_date": "2023-12-31",
        "sentiment_delta": 0.06,
        "title_zh": "AI 概念潮",
        "title_en": "The AI Tide",
        "prose_zh": "大模型发布点燃算力板块，卖方连夜改模型，散户连夜改备注。",
        "prose_en": "A large-model launch ignites compute names; analysts rewrite models overnight and retail investors rewrite their watchlists.",
    },
    {
        "id": "bs06",
        "from_date": "2024-02-01",
        "to_date": "2024-04-30",
        "sentiment_delta": -0.07,
        "title_zh": "监管窗口",
        "title_en": "The Regulatory Window",
        "prose_zh": "交易所加强异常交易监控，部分席位被限制交易，市场短暂安静了一整天。",
        "prose_en": "Exchanges tighten surveillance on abnormal trading, several seats are restricted, and the market goes quiet for a day.",
    },
    {
        "id": "bs07",
        "from_date": "2024-05-01",
        "to_date": "2024-12-31",
        "sentiment_delta": 0.04,
        "title_zh": "玄学行情",
        "title_en": "The Mysticism Rally",
        "prose_zh": "市场开始认真讨论生肖、星座与 K 线形状，连分析师都分不清谁在开玩笑。",
        "prose_en": "The market seriously debates zodiac signs, star signs, and candle shapes, until even analysts cannot tell who is joking.",
    },
    {
        "id": "bs08",
        "from_date": "2025-01-01",
        "to_date": "2025-12-31",
        "sentiment_delta": 0.05,
        "title_zh": "估值修复共识",
        "title_en": "The Repair Consensus",
        "prose_zh": "“估值修复”成为主流叙事，连出租车司机都开始纠正别人的仓位结构。",
        "prose_en": "\"Valuation repair\" becomes the mainstream story, and even taxi drivers start correcting other people's position sizing.",
    },
    {
        "id": "bs09",
        "from_date": "2026-06-01",
        "to_date": "2026-07-31",
        "sentiment_delta": 0.09,
        "title_zh": "算力订单潮",
        "title_en": "The Compute Order Flood",
        "prose_zh": "多家科技公司发布算力订单相关消息，板块放量上涨，营业部里全是新面孔。",
        "prose_en": "Several tech firms release news tied to compute orders; the sector surges on volume and the brokerage floor fills with new faces.",
    },
    {
        "id": "bs10",
        "from_date": "2026-07-01",
        "to_date": "2026-08-31",
        "sentiment_delta": -0.07,
        "title_zh": "高位分歧",
        "title_en": "Divergence at the Top",
        "prose_zh": "龙头冲高回落，龙虎榜买卖双方互道保重，有人开始把麻袋换成现金。",
        "prose_en": "The leader spikes and fades; the buy and sell seats wish each other well as some start trading sacks for cash.",
    },
    {
        "id": "bs11",
        "from_date": "2021-07-01",
        "to_date": "2021-07-31",
        "sentiment_delta": 0.05,
        "title_zh": "央行降准落地",
        "title_en": "The Reserve-Ratio Cut",
        "prose_zh": "央行宣布全面降准，市场解读为流动性宽松信号，券商营业部连夜改标题。",
        "prose_en": "The central bank announces a broad reserve-ratio cut; the market reads it as liquidity easing and brokerages rewrite their headlines overnight.",
    },
    {
        "id": "bs12",
        "from_date": "2022-06-01",
        "to_date": "2022-12-31",
        "sentiment_delta": -0.08,
        "title_zh": "美联储激进加息",
        "title_en": "The Fed Hikes Hard",
        "prose_zh": "海外央行激进加息，全球风险资产承压，人民币汇率与外资流向成为每天的新头条。",
        "prose_en": "Overseas central banks hike aggressively, risk assets come under pressure, and the yuan and foreign flows become the daily headline.",
    },
    {
        "id": "bs13",
        "from_date": "2023-02-01",
        "to_date": "2023-04-30",
        "sentiment_delta": 0.06,
        "title_zh": "全面注册制落地",
        "title_en": "Full Registration System",
        "prose_zh": "全面注册制正式实施，新股定价规则重塑，打新一族和主板老股民同时更新自己的玩法手册。",
        "prose_en": "The full registration system takes effect, repricing rules are reshaped, and both new-issue hunters and main-board veterans update their playbooks.",
    },
    {
        "id": "bs14",
        "from_date": "2024-04-01",
        "to_date": "2024-05-31",
        "sentiment_delta": 0.05,
        "title_zh": "新国九条",
        "title_en": "The New Nine Articles",
        "prose_zh": "资本市场纲领性文件发布，分红、退市与长期资金成为关键词，蓝筹股集体抬头。",
        "prose_en": "A new capital-market guideline lands; dividends, delisting, and long-term money become the keywords and blue chips lift together.",
    },
    {
        "id": "bs15",
        "from_date": "2025-06-01",
        "to_date": "2025-10-31",
        "sentiment_delta": -0.07,
        "title_zh": "关税摩擦升温",
        "title_en": "Tariff Friction Heats Up",
        "prose_zh": "国际贸易摩擦再度升温，出口链股票剧烈波动，买方研究员开始背各国税率表。",
        "prose_en": "International trade friction flares again, export chains swing hard, and buy-side analysts start memorizing tariff tables.",
    },
    {
        "id": "bs16",
        "from_date": "2026-01-01",
        "to_date": "2026-03-31",
        "sentiment_delta": -0.06,
        "title_zh": "地缘局势紧张",
        "title_en": "Geopolitical Tension",
        "prose_zh": "地缘局势骤然紧张，避险资金涌向黄金与债券，风险偏好被按回地面。",
        "prose_en": "Geopolitical tension spikes; money rushes into gold and bonds while risk appetite gets pinned to the floor.",
    },
    {
        "id": "bs17",
        "from_date": "2026-04-01",
        "to_date": "2026-07-31",
        "sentiment_delta": 0.06,
        "title_zh": "降息周期开启",
        "title_en": "The Rate-Cut Cycle Begins",
        "prose_zh": "政策利率下调，市场开始重估所有长久期资产，成长股与科技股闻风而动。",
        "prose_en": "Policy rates fall, every long-duration asset gets repriced, and growth and tech names stir at the first whisper.",
    },
]


def eligible_black_swans(date: str):
    return [
        event
        for event in BLACK_SWANS
        if event["from_date"] <= date <= event["to_date"]
    ]


def pick_black_swan(date: str, rng: random.Random | None = None):
    pool = eligible_black_swans(date)
    if not pool:
        return None
    chooser = rng or random
    return chooser.choice(pool)


def localize_black_swan(event: dict, lang: str) -> dict:
    zh = lang == "zh"
    return {
        "id": event["id"],
        "date": event.get("date"),
        "title": event["title_zh"] if zh else event["title_en"],
        "prose": event["prose_zh"] if zh else event["prose_en"],
        "sentiment_delta": event["sentiment_delta"],
    }


def black_swan_options(lang: str) -> list:
    zh = lang == "zh"
    return [
        {
            "key": "buy_dip",
            "label": "抢反弹" if zh else "Buy the dip",
            "detail": "现金 -1,500 · 情绪 +3%" if zh else "Cash -1,500 · Sentiment +3%",
            "effect": {"cash": -1500, "sentiment": 0.03},
        },
        {
            "key": "hold",
            "label": "按兵不动" if zh else "Stand pat",
            "detail": "无效果" if zh else "No effect",
            "effect": {"cash": 0, "sentiment": 0},
        },
        {
            "key": "take_profit",
            "label": "落袋为安" if zh else "Take profit",
            "detail": "现金 +1,000 · 情绪 -2%" if zh else "Cash +1,000 · Sentiment -2%",
            "effect": {"cash": 1000, "sentiment": -0.02},
        },
    ]
