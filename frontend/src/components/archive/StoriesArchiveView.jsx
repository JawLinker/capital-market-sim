    ...
    <div className="space-y-4 p-4 lg:p-5">
        ...
        <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <h2 className="text-sm font-semibold text-slate-100">{t("archive.title")}</h2>
              <p className="mt-0.5 text-xs text-slate-500">{t("archive.detail")}</p>
            </div>
          </div>
          <select
            value={eraFilter}
      ...
      {visible.length === 0 ? (
        <section className="panel flex flex-col items-center gap-2 p-10 text-center">
          ...
          <p className="text-sm text-slate-400">{t("archive.empty")}</p>
        </section>
      ) : (
            ...
            return (
              <article key={story.id} className="panel relative overflow-hidden">
                ...
                <div className="space-y-3 p-4">
                  ...
                  <h3 className="text-base font-bold leading-6 text-slate-50">{story.title}</h3>
